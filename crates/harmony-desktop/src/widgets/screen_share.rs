...
pub struct ScreenShareProgram {
    frame: Arc<ArcSwap<Option<CaptureFrame>>>,
}

impl ScreenShareProgram {
    pub fn new(frame: Arc<ArcSwap<Option<CaptureFrame>>>) -> Self {
        ScreenShareProgram { frame }
    }
}

impl<Message> Program<Message> for ScreenShareProgram {
    type State = ();
    type Primitive = ScreenSharePrimitive;
    ...
    fn draw(
        ...
        ScreenSharePrimitive {
            frame: Arc::clone(&self.frame),
        }
    }
}

pub struct ScreenSharePrimitive {
    pub frame: Arc<ArcSwap<Option<CaptureFrame>>>,
}

impl std::fmt::Debug for ScreenSharePrimitive {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ScreenSharePrimitive")
            .finish_non_exhaustive()
    }
}

impl Primitive for ScreenSharePrimitive {
    type Pipeline = ScreenSharePipeline;

    fn prepare(
        &self,
        pipeline: &mut ScreenSharePipeline,
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        _bounds: &Rectangle,
        _viewport: &Viewport,
            ...
            .import(capture_frame, device, queue, &desc)
        {
            ...
            Err(e) => {
                tracing::debug!("screen share import frame: {e}");
            }
        }
    }
...
impl Pipeline for ScreenSharePipeline {
    fn new(device: &wgpu::Device, _queue: &wgpu::Queue, format: wgpu::TextureFormat) -> Self {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("screen_share_pl"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("screen_share_shader"),
            source: wgpu::ShaderSource::Wgsl(SHADER_WGSL.into()),
        });

        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("screen_share_rp"),
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("screen_share_sampler"),
